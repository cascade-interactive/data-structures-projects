...
// Helper function to double check user entered an integer and not something
// else
...
// Function for searching for hats in the collection by prompting the user for a
// search field and then asking for the value to search for. It will then call
// the search function on the hat collection and display the results.
void prompt_hat_search(const CircularList<Hat> &hat_collection) {
    ...
    while (true) {
        ...
        cout
            << "Invalid selection. Please choose an option from 1 through 4.\n";
    }
    ...
    case 4: {
        ...
        cout << "\nSearch Results\n";
        cout << "--------------\n";
        match_count = hat_collection.search([coolness_level](const Hat &hat) {
...
// Function for adding hats to the collection by asking the user for the amount
// of hats and then prompting them for the color, brand, and coolness for each.
// It will then insert the hat into the collection and increment the next_id for
// the next hat to be added.
...
// Function that will capitalize the first letter of a string and make the rest
// lowercase mainly used for formatting for the color and brand
...
// A carousel for the hats that allows the user to view the hats in the
// collection and move through the linked list with the next and previous
// options. It will display the current hat and the total number of hats in the
// collection.
void HatCarousel::run(size_t count) {
    ...
    do {
        ...
        default:
            cout << "Invalid selection. Please choose an option from 1 through "
                    "3.\n";
            break;
        }
    } while (choice != 3);
